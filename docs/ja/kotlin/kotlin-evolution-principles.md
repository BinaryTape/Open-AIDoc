...
* `-language-version X.Y` – Kotlin 言語バージョン X.Y に対する互換モード。以降のバージョンで導入された言語機能をコードが使用している場合、コンパイラはエラーを報告します。
* `-api-version X.Y` – Kotlin API バージョン X.Y に対する互換モード。コンパイラによって生成されたコードで参照される API を含め、以降のバージョンで導入された Kotlin 標準ライブラリの API を使用している宣言をコンパイラは無視します。
...
アクティブにメンテナンスされているコードベースは、完全な非推奨サイクルが完了するのを待たずに、できるだけ早くバグ修正の恩恵を受けることができます。そのようなプロジェクトでは [`-progressive` オプション](compiler-reference.md#progressive) を有効にすることで、それらがデフォルトになる前のツールリリースであっても、そのような変更を採用できます。
